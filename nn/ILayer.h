...
struct ILayer{
  ...
  //逆伝播
  //次の層の勾配を受け取る
  virtual Tensor backward(const Tensor& grad_output_tensor) = 0;

  //逆伝播の時最後の層は勾配計算がいらないため
  virtual Tensor backward_no_grad(const Tensor& grad_output_tensor){
    return backward(grad_output_tensor);
  }
